...
const LeaveCalendarView: React.FC<LeaveCalendarViewProps> = ({ currentDate, staff, leaveRequests, leaveTypes }) => {
    ...
    const dailyLeave = useMemo(() => {
        ...
        for (let day = 1; day <= daysInMonth; day++) {
            ...
            const leavesForDay = leaveRequests
                ...
                .filter(item => !!item.staff); // Ensure staff member is in the current filtered list
        ...
        return map;
    }, [year, month, daysInMonth, leaveRequests, staffMap]);
                        ...
                        {leaves.map(({ request, staff }) => {
                            ...
                            return (
                                <div 
                                    ...
                                    title={`${staff?.name}: ${leaveType?.name} (${request.status})`} 
                                    className="text-xs p-1 rounded-md text-white truncate" 
                                    style={{ backgroundColor: leaveType?.color || '#777' }}
                                >
                                    <span className={`inline-block w-2 h-2 mr-1.5 rounded-full ${request.status === 'approved' ? 'bg-white' : 'bg-yellow-300'}`}></span>
                                    {staff?.name}
                                </div>
                            );
                        })}
